...
export default function SubmissionCard({
  ...
  // Check if submission is starred (only for signed in users)
  useEffect(() => {
    if (!isSignedIn) {
      setIsStarred(false)
      return
  ...
  async function saveToBoard(boardId: string | null) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // If a board was selected, move it there
        if (boardId) {
          await fetch('/api/user/saved', {
            method: 'PATCH',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ submission_id: submission.id, board_id: boardId }),
          })
        }
      }
    } catch (error) {
  ...
  return (
    ...
    >
      <div className="relative">
        ...
        {/* Star button - top right (only for signed in users) */}
        {isSignedIn && (
          <div className="absolute top-2 right-2">
            <button
              ...
              title={isStarred ? 'Remove from saved' : 'Save to board'}
            >
              {isStarring ? (
                <svg className={`${isCompact ? 'w-3 h-3' : 'w-4 h-4'} animate-spin`} viewBox="0 0 24 24" fill="none">
